...
## Pre-Work Context Check (필수)

모든 프로젝트 작업 전에 다음 순서로 context를 확인한다:

1. `ai/session-logs/` — 가장 최근 세션 로그로 최신 변경사항과 결정을 파악
2. `ai/plans/product-plan.md` — 최초 기획문 및 Change Log (추가 기획문)
3. `ai/plans/design-plan.md` — 최초 디자인기획문 및 Change Log (추가 기획문)
4. `00_PROJECT_BRIEF.md` / `01_DECISIONS.md` — 프로젝트 제약과 승인 결정

중간 변경사항 발생 시:
- 기획 변경 → `ai/plans/product-plan.md` 본문 + Change Log
- 디자인 변경 → `ai/plans/design-plan.md` 본문 + Change Log
- 구현 변경 → `ai/plans/implementation-plan.md` 본문 + Change Log
- 모든 의미 있는 세션 → `ai/session-logs/` 날짜별 로그

이 규칙은 시장조사, 리서치, QA, 리뷰, 기획 제안 등 모든 활동에 적용된다.
...
- When reusable knowledge appears during `무한칭찬앱` work, promote it into Markdown under `/Users/kangsungbae/Documents/지식저장소` so Codex, Claude, and future AI assistants can share it.
...
- `무한칭찬앱` is a non-game app, so use Apps in Toss as the first release target while keeping Google Play compatibility intact.
