...
export function classifyMessageSafety(text: string): { level: SafetyLevel; reason?: SafetyReason } {
  const reason = getBlockedReason(text);
  if (reason) return { level: "blocked", reason };
  if (cautionPatterns.some((pattern) => text.includes(pattern)) || text.trim().length > 0 && text.trim().length < 8) {
    return { level: "caution", reason: "abuse" };
  }
  return { level: "safe" };
}
